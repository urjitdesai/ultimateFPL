...
const emptyView: PredictionView = { fixtures: [], summary: { totalPoints: 0, gameweekPoints: 0, submittedCount: 0, fixtureCount: 0 } };
...
export function HomePage() {
  ...
  return <main className="home-page">
    ...
    <section className="home-content"><div className="fixtures-section">
      <header><div><h2>{selected?.status === "COMPLETE" ? "Your results" : "Your predictions"}</h2><p>{selected?.status === "COMPLETE" ? "Prediction, final score and points" : "Every match locks independently at kickoff"}</p></div><div className="fixture-actions"><span className="fixture-count">{view.fixtures.length} matches</span>{selected?.status !== "COMPLETE" ? <button className={`save-predictions ${saved ? "is-saved" : ""}`} disabled={saving || savableCount === 0} onClick={savePredictions}>{saved ? <Check /> : <Save />}{saving ? "Saving…" : saved ? "Saved" : `Save ${savableCount || ""} predictions`}</button> : null}</div></header>
