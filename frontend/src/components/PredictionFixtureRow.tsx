...
import { playerWagerResultLabel } from "../player-wager";
...
export function PredictionFixtureRow({ fixture, draft, kickoff, isCaptain, onChange, onCaptain, wager, wagerUnavailableReason, wagerDraft, wagerBalance, wagerEligible, hasOtherWager, currentWagerStake, onSetWager, onRemoveWager }: Props) {
  ...
  return <article className={`fixture-row prediction-row ${fixture.predictionLocked ? "is-locked" : ""}`}>
    ...
    <div className="prediction-cell">
      ...
      </div>{wagerSettled && wager ? <span className={`prediction-wager-summary ${wager.status.toLowerCase()}`}><Coins />{playerWagerResultLabel(wager)} · {selectionLabel(wager.selection, fixture)}</span> : null}</div> : fixture.predictionLocked ? <div className="locked-prediction-wrap"><div className="locked-prediction"><LockKeyhole /><span>{prediction ? prediction.isCaptain ? "Captain pick" : "Your pick" : "Predictions closed"}</span><strong>{prediction ? `${prediction.predictedHomeScore}–${prediction.predictedAwayScore}` : "—"}</strong></div>{wager ? <span className="prediction-wager-summary"><Coins />{wager.stakePoints} pts on {selectionLabel(wager.selection, fixture)}</span> : null}</div> : <div className="prediction-editor">
        <div className="score-entry" aria-label={`Prediction for ${fixture.homeTeam.name} against ${fixture.awayTeam.name}`}><input aria-label={`${fixture.homeTeam.name} predicted score`} inputMode="numeric" min="0" max="20" type="number" value={draft.home} onWheel={(event) => event.currentTarget.blur()} onFocus={() => { if (draft.home === "0") onChange(fixture.id, "home", ""); }} onBlur={() => { if (draft.home === "") onChange(fixture.id, "home", "0"); }} onChange={(event) => onChange(fixture.id, "home", normalizeScoreInput(event.target.value))} /><span>–</span><input aria-label={`${fixture.awayTeam.name} predicted score`} inputMode="numeric" min="0" max="20" type="number" value={draft.away} onWheel={(event) => event.currentTarget.blur()} onFocus={() => { if (draft.away === "0") onChange(fixture.id, "away", ""); }} onBlur={() => { if (draft.away === "") onChange(fixture.id, "away", "0"); }} onChange={(event) => onChange(fixture.id, "away", normalizeScoreInput(event.target.value))} /></div>
        <div className="prediction-options"><button className={`captain-button ${isCaptain ? "is-captain" : ""}`} type="button" disabled={draft.home === "" || draft.away === ""} aria-pressed={isCaptain} onClick={() => onCaptain(fixture.id)}><Crown />Captain</button>{wagerDraft || wager ? <button className="captain-button wager-button is-wager" type="button" aria-pressed="true" title="Remove wager" onClick={onRemoveWager}><Coins />Wager</button> : <button className="captain-button wager-button" type="button" disabled={!canAddWager} aria-pressed="false" title={wagerButtonTitle} onClick={() => { if (predictedSelection) onSetWager({ fixtureId: fixture.id, selection: predictedSelection, stakePoints: hasOtherWager ? currentWagerStake : 1 }); }}><Coins />Wager</button>}{wagerDraft ? <label className="wager-points-input"><input aria-label={`Points to wager on ${fixture.homeTeam.name} versus ${fixture.awayTeam.name}`} type="number" inputMode="numeric" enterKeyHint="done" min="1" max={Math.min(MAX_WAGER_POINTS, maxAvailableStake)} value={wagerDraft.stakePoints} onWheel={(event) => event.currentTarget.blur()} onChange={(event) => { const stakePoints = Math.min(MAX_WAGER_POINTS, Math.max(1, Number(event.target.value))); onSetWager({ fixtureId: fixture.id, selection: predictedSelection ?? wagerDraft.selection, stakePoints }); }} /><span>pts</span></label> : wager ? <span className="wager-points-value">{wager.stakePoints} pts</span> : null}</div>
      </div>}
    </div>
