import { ArrowRight, Check, Copy, KeyRound, Plus, Users } from "lucide-react";
  ...
  return <main className="league-page">
    ...
    <section className="league-directory">
      <div className="league-actions">
        <form className="league-action-card" onSubmit={createCustomLeague}><span className="league-action-icon"><Plus /></span><div><h2>Create a league</h2><p>Name your competition and get a key to share.</p><label><span>League name</span><input value={name} maxLength={50} placeholder="Friday Night Rivals" onChange={(event) => setName(event.target.value)} /></label><button disabled={submitting !== null || name.trim().length < 3}>{submitting === "create" ? "Creating…" : "Create league"}</button></div></form>
      ...
      <div className="league-directory-head"><span>League</span><span>Rules</span><span>Members</span><span /></div>
      {loading ? <div className="league-loading">Loading league tables…</div> : error ? <div className="home-error" role="alert">{error}<button onClick={() => window.location.reload()}>Retry</button></div> : leagues.map((league) => {
        return <button className="league-directory-row" key={league.id} onClick={() => navigate(`/leagues/${encodeURIComponent(league.id)}`)}>
          ...
          <span className="league-type">Outcome wagers</span>
