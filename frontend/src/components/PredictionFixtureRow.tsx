...
export function PredictionFixtureRow({ fixture, draft, kickoff, isCaptain, onChange, onCaptain }: Props) {
  ...
  return <article className={`fixture-row prediction-row ${fixture.predictionLocked ? "is-locked" : ""}`}>
    ...
    <div className="prediction-cell">
      ...
      </div> : fixture.predictionLocked ? <div className="locked-prediction"><LockKeyhole /><span>{prediction ? prediction.isCaptain ? "Captain pick" : "Your pick" : "Predictions closed"}</span><strong>{prediction ? `${prediction.predictedHomeScore}–${prediction.predictedAwayScore}` : "—"}</strong></div> : <div className="prediction-editor">
