...
export function RegisterPage() {
  ...
  const submit = handleSubmit(async (values) => {
    ...
    try {
      ...
      const profile = await api.registerProfile(user, values.displayName);
      setProfile(profile);
      navigate("/dashboard");
    } catch (error) {
      setPageError(error instanceof Error ? error.message.replace("Firebase: ", "") : "We couldn't create your account.");
    }
  });

  return <AuthShell><div className="auth-card">
    <header className="form-header"><div><h2>Create your account</h2><p>One minute to your first wager.</p></div><span className="secure-note"><ShieldCheck /> Secure signup</span></header>
    <form onSubmit={submit} noValidate>
      ...
      <div className="league-preview selected"><span className="team-monogram"><Coins /></span><div><strong>100 free starting points</strong><p>Pick match outcomes and wager up to 20 points per fixture.</p></div></div>
